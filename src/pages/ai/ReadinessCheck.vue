      ...
      <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <transition name="fade" mode="out-in">
          <div v-if="step === 1 && !showResult" key="step1" class="bg-white dark:bg-surface-200 rounded-[var(--radius-card)] shadow-[var(--shadow-card)] p-6">
            <h2 class="text-xl font-bold text-surface-900 dark:text-surface-950 mb-4">In welcher Branche ist Ihr Unternehmen tätig?</h2>
            <div class="grid gap-3">
              <button v-for="option in branchen" :key="option" @click="answers.branche = option; step = 2"
                ...
                :class="answers.branche === option ? 'border-primary-500 bg-primary-50' : 'border-surface-200'">
                {{ option }}
              </button>
            </div>
          </div>
          ...
          <div v-else-if="step === 2 && !showResult" key="step2" class="bg-white dark:bg-surface-200 rounded-[var(--radius-card)] shadow-[var(--shadow-card)] p-6">
            <h2 class="text-xl font-bold text-surface-900 dark:text-surface-950 mb-4">Wie viele Mitarbeiter hat Ihr Unternehmen?</h2>
            <div class="grid gap-3">
              <button v-for="option in mitarbeiterOptions" :key="option" @click="answers.mitarbeiter = option; step = 3"
                class="text-left px-4 py-3 rounded-lg border-2 transition text-surface-700 dark:text-surface-800 hover:border-primary-500 hover:bg-primary-50"
                :class="answers.mitarbeiter === option ? 'border-primary-500 bg-primary-50' : 'border-surface-200'">
                {{ option }}
              </button>
            </div>
          ...
          <div v-else-if="step === 3 && !showResult" key="step3" class="bg-white dark:bg-surface-200 rounded-[var(--radius-card)] shadow-[var(--shadow-card)] p-6">
            <h2 class="text-xl font-bold text-surface-900 dark:text-surface-950 mb-2">Welche wiederkehrenden manuellen Prozesse haben Sie?</h2>
            <p class="text-sm text-surface-400 mb-4">Mehrfachauswahl möglich</p>
            <div class="grid gap-3">
              <button v-for="option in prozessOptions" :key="option" @click="toggleProzess(option)"
                class="text-left px-4 py-3 rounded-lg border-2 transition text-surface-700 dark:text-surface-800 hover:border-primary-500 hover:bg-primary-50"
                :class="answers.prozesse.includes(option) ? 'border-primary-500 bg-primary-50' : 'border-surface-200'">
                {{ option }}
              </button>
            </div>
            <div class="flex justify-between mt-4">
              <button @click="step = 2" class="text-sm text-surface-400 hover:text-surface-600">&larr; Zurück</button>
              <button @click="step = 4" :disabled="answers.prozesse.length === 0"
                class="bg-primary-500 hover:bg-primary-600 disabled:bg-surface-300 text-white font-semibold py-2 px-6 rounded-[var(--radius-button)] transition">
                Weiter &rarr;
              </button>
            </div>
          </div>
          ...
          <div v-else-if="step === 4 && !showResult" key="step4" class="bg-white dark:bg-surface-200 rounded-[var(--radius-card)] shadow-[var(--shadow-card)] p-6">
            <h2 class="text-xl font-bold text-surface-900 dark:text-surface-950 mb-4">Wie sieht Ihre IT-Infrastruktur aus?</h2>
            <div class="grid gap-3">
              <button v-for="option in infraOptions" :key="option" @click="answers.infra = option; step = 5"
                class="text-left px-4 py-3 rounded-lg border-2 transition text-surface-700 dark:text-surface-800 hover:border-primary-500 hover:bg-primary-50"
                :class="answers.infra === option ? 'border-primary-500 bg-primary-50' : 'border-surface-200'">
                {{ option }}
              </button>
            </div>
          ...
          <!-- Step 5: Budget -->
          <div v-else-if="step === 5 && !showResult" key="step5" class="bg-white dark:bg-surface-200 rounded-[var(--radius-card)] shadow-[var(--shadow-card)] p-6">
            <h2 class="text-xl font-bold text-surface-900 dark:text-surface-950 mb-4">Welches Budget haben Sie für Automatisierung?</h2>
            <div class="grid gap-3">
              <button v-for="option in budgetOptions" :key="option" @click="answers.budget = option; evaluate()"
                class="text-left px-4 py-3 rounded-lg border-2 transition text-surface-700 dark:text-surface-800 hover:border-primary-500 hover:bg-primary-50"
                :class="answers.budget === option ? 'border-primary-500 bg-primary-50' : 'border-surface-200'">
                {{ option }}
              </button>
            </div>
            <button @click="step = 4" class="mt-4 text-sm text-surface-400 hover:text-surface-600">&larr; Zurück</button>
          </div>
            ...
            <div class="text-center mb-6">
              <div class="relative inline-flex items-center justify-center w-28 h-28 mb-4">
                <svg class="w-28 h-28 -rotate-90" viewBox="0 0 120 120">
                  <circle cx="60" cy="60" r="52" fill="none" stroke="var(--color-surface-200)" stroke-width="8" />
                ...
                <span class="absolute text-2xl font-bold text-surface-900 dark:text-surface-950">{{ score }}</span>
              </div>
              <h2 class="text-2xl font-bold" :style="{ color: scoreColor }">{{ level }}</h2>
            </div>

            <div class="grid gap-4 mb-6">
              <div class="flex justify-between items-center p-3 bg-surface-50 dark:bg-surface-100 rounded-lg">
                <span class="text-surface-500">Readiness-Level</span>
                <span class="font-bold text-surface-900 dark:text-surface-950">{{ level }}</span>
              </div>
              <div class="flex justify-between items-center p-3 bg-surface-50 dark:bg-surface-100 rounded-lg">
                <span class="text-surface-500">Geschätzte Zeitersparnis</span>
                <span class="font-bold text-surface-900 dark:text-surface-950">{{ timeSaving }} Std./Woche</span>
              </div>
              <div class="flex justify-between items-center p-3 bg-surface-50 dark:bg-surface-100 rounded-lg">
                <span class="text-surface-500">Empfohlenes Paket</span>
                <span class="font-bold text-surface-900 dark:text-surface-950">{{ recommendedPackage }}</span>
              </div>
            </div>

            <p class="text-surface-600 mb-6 leading-relaxed">{{ recommendation }}</p>
